...
{
    class LessOrEqualsN : BinaryRelation, IMetricRelation
    {
        public LessOrEqualsN()
            : base(
                     MetricRelationNames.LessOrEqualsN,
                     "B <= n",
        ...
        public override List<TmsConstraint> GetTmsConstraints(ConfigurationConstraint constraint, Edge edge)
        {
            ...
            else
            {
                ...
                // Greater than constraint type is used with switched places of the relations elements  => less than
                tmsConstraint.ConstraintType = TmsManager.CTNameGreaterOrEquals;
                tmsConstraint.VariableTuple = new List<string>() { dVarMetric, dVarA, constraint.GetSatDVarName() };
        ...
        {
            // A - BeginOfTime is in [MIN, n]
            MetricRelationPart relPartN = constraint.RelationParts[1] as MetricRelationPart;
            Interval interval = new Interval(StructuralRelationsManager.MetricDomain.MinValue , relPartN.GetIntValue());
